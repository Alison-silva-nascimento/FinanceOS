...
a,b,c=st.columns(3); a.metric("Aplicação",APP_NAME); b.metric("Versão",APP_VERSION); c.metric("Dados", "Supabase" if usar_postgres() else "Locais")
...
if usar_postgres():
    st.info("Os dados de produção estão no Supabase. O arquivo SQLite local não é usado nesta base; utilize a exportação CSV abaixo ou os recursos de backup do Supabase.")
else:
...
if not usar_postgres():
    with st.expander("♻️ Restaurar backup"):
        ...
        if st.button("Validar e restaurar", disabled=not arquivo_backup or confirmacao != "RESTAURAR"):
            ...
            except (ValueError, OSError, RuntimeError) as erro: st.error(str(erro))
